...
for i in range(0,numeroestrellas):
    ...
    estrellas[-1].setVelocidad(random.randint(1,5))
...
def bucle():
    for i in range (0,len(estrellas)):
        x,y = estrellas[i].getPosicion()
        estrellas[i].setPosicion(x,y+estrellas[i].getVelocidad())
        lienzo.coords(formaestrellas[i],x,y+estrellas[i].getVelocidad(),x+2,y+2)
        if y+5 > 512:
            estrellas[i].setPosicion(x,0)
    for bala in balas:
